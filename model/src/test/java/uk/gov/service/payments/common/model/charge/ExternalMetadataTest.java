package uk.gov.service.payments.common.model.charge;
...
public class ExternalMetadataTest {
    ...
    private final String TOO_LONG_VALUE = IntStream.rangeClosed(1, ExternalMetadata.MAX_VALUE_LENGTH + 1).mapToObj(i -> "a").collect(joining());
    private final String TOO_LONG_KEY = IntStream.rangeClosed(1, ExternalMetadata.MAX_KEY_LENGTH + 1).mapToObj(i -> "a").collect(joining());
    ...
    public void shouldFailValidationWithMoreThan10Keys() {
        ...
        assertThat(violations.iterator().next().getMessage(), Matchers.is("Field [metadata] cannot have more than " + ExternalMetadata.MAX_KEY_VALUE_PAIRS + " key-value pairs"));
    }
    ...
    public void shouldFailValidationForKeysToLong() {
        ...
        assertThat(violations.iterator().next().getMessage(),
                Matchers.is("Field [metadata] keys must be between " + ExternalMetadata.MIN_KEY_LENGTH + " and " + ExternalMetadata.MAX_KEY_LENGTH + " characters long"));
    }
    ...
    public void shouldFailValidationForValueToLong() {
        ...
        assertThat(violations.size(), is(1));
        assertThat(violations.iterator().next().getMessage(),
                Matchers.is("Field [metadata] values must be no greater than " + ExternalMetadata.MAX_VALUE_LENGTH + " characters long"));
    }
    ...
    public void shouldFailWithMultipleViolations() {
        ...
        Set<String> expectedErrorMessages = Set.of(
                ...
                "Field [metadata] keys must be between " + ExternalMetadata.MIN_KEY_LENGTH + " and " + ExternalMetadata.MAX_KEY_LENGTH + " characters long");
