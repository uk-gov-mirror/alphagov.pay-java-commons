...
public class PayInteractionRunner extends InteractionRunner {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(PayInteractionRunner.class);
    ...
    private void init(BrokerUrlSource brokerUrlSource) {
        ...
        Assert.assertNotNull("The pact got from the broker has no pb:publish-verification-results property.", attributes.get("pb:publish-verification-results"));
        Assert.assertNotNull("pb:publish-verification-results has no href property", attributes.get("pb:publish-verification-results").get("href"));
        ...
        Assert.assertTrue(auth.size() == 3);
        username = (String) auth.get(1);
        password = (String) auth.get(2);
        Assert.assertNotNull("No auth username to the pact broker provided", username);
        Assert.assertNotNull("No auth password to the pact broker provided", password);
